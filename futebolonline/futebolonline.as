...
//vars
{
	...
	var car6 = "Atirando copos (de plástico) no jogadores!";
...
recebeListaDeJogadores_xml.onLoad = function () {
	...
	//
	//set(camp, valor);
	//todosOsJogadores_array = todosOsJogadores_array.slice(0,0);//if(isNaN(numeroJogadores) || numeroJogadores==0)
...
recebeJogaAvenda1_xml.onLoad = function () {
	...
	for(u2=1; u2<numeroJogaAvendadoresAvenda; u2++) {
		...
		if(jogaAvendatimeAtual!=timeAtual && jogaAvendatimeAtual!=undefined && jogaAvendatimeAtual!="undefined")
...
recebeAddtime_xml.onLoad = function () {
	...
	if(msg_txt.text=="Registrado") {
		if(nometime_txt.text=="podeCriar" || nometime_txt.text=="sem time" || nometime_txt.text=="jogando" || nometime_txt.text=="Palmeiras" || nometime_txt.text=="Brasil" || nometime_txt.text=="Corinthians" || nometime_txt.text=="São Paulo" || nometime_txt.text=="Santos") {
...
function logar() {
	...
	//
	if (cookieSetado && cookieConteudo.toString()==ConteudoQueColocarasNoCookie) {
...
function verificaSeEstaVerificandoCampo() {
	verificampoInt = setInterval(function () {
		...
		if(recebeJogaAvenda_carregando || recebeConta_carregando || recebeJoga_carregando || recebeCookie_carregando || recebeTime_carregando || recebeSala_carregando || recebeEstadio_carregando || recebeJSON_carregando) {
			stop();
		} else {
...
function esperarAlguemEntrarNaSala(hospedaria) {
	...
	esperarAlguemEntrarNaSalainterval = setInterval(function () {
		...
		//if(vrs3!="vazia" && vrs3!=undefined && vrs3!="") {
			if(timeAdversario!="vazia" && timeAdversario!=undefined && timeAdversario!="") {
...
function verificaCampoEstadio(campo) {
	...
	if(estadioNome!="sem estadio" && estadioNome!="" && estadioNome!=undefined) {
...
function respondeu(resp) {
	//trace(resp);
	//trace(respostacerta);
	if(resp.toString()==respostacerta.toString() || respostacerta>4) {
		acertos++;
...
function loadSceneRegistro() {
	...
	function registrar() {
		...
		} else if (email_var==undefined || !email_bol) {
			//trace(email_bol);
...
function loadSceneCriarTime() {
	...
	setarCor(seletorsec_mc, "0x00FF00", seletorpr_mc._x, seletorpr_mc._y);
	//
...
function loadSceneIngame() {
	...
	sortearResultado = function() {
		...
		if(golsQueFaras>=6 && golsQueTomaras<golsQueFaras) {
			//trace("Fará mais de 6 gols e vai ganhar");
			golsQueFaras = 3*sorte;
			golsQueTomaras = sorte*sorte;
		} else if(golsQueTomaras>=6 && golsQueFaras<golsQueTomaras) {
			//trace("Tomará mais de 6 gols e vai perder");
			golsQueTomaras = 6-sorte;
			golsQueFaras = 3+sorte;
		} else if(golsQueTomaras==golsQueFaras && golsQueTomaras>3) {
	...
	function resultador() {
		...
		intervalo = setInterval( function () {
			...
			if(tempo<250 || (golsFeitos==golsQueFaras && golsTomados==golsQueTomaras))
			...
			if(golsFeitos==golsQueFaras && golsTomados==golsQueTomaras) {
				time1forca = time2forca = 100;
			}
			//Quando a barra acerta o ponto maximo do GOL
			if(barra1_mc._width<gol1_txt._x-20 || barra1_mc._width>gol2_txt._x+20) {
	...
	function gol(fezGol) {
		...
		/*if (tempo>tempoPartida/2) {
			...
			if(golsFeitos<golsQueFaras && golsTomados<golsQueTomaras) {
				...
				//trace(boolean);
			} else if (golsFeitos<golsQueFaras && golsTomados>=golsQueTomaras) {
	...
	function pausar(tempo) {
		...
		voltarAoJogo = setInterval(function () {
			...
			//if(jogador1Pronto && jogador2Pronto) {
				resultador();
...
function aqui_mc_load_salas(filtroForca, filtroSenha, filtroComputador) {
	...
	for(i=0; i<jsonObject.salas.length;i++) {
		...
		if(jsonObject.salas[i].senha=="" || jsonObject.salas[i].senha=="undefined" || jsonObject.salas[i].senha==undefined)
