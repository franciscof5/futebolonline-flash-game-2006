...
#var_dump($con);
#var_dump($_POST);die;
$result = mysqli_query($con, "SELECT userID,pass,tecnico,timeAtual FROM cadastros WHERE user='$usuario' LIMIT 1");
if($result) {
	while ($row = mysqli_fetch_array($result)) {
		if($senha == $row["pass"]) {
			echo "<xmlbacia><aceito>1</aceito><tecnico>".$row['tecnico']."</tecnico><userID>".$row['userID']."</userID><timeAtual>".$row['timeAtual']."</timeAtual></xmlbacia>";
		}
