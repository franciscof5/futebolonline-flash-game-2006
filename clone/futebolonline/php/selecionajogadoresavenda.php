...
$buscajogadores3 = mysql_query("SELECT * FROM `jogadores` WHERE avenda=1 ");
$buscajogadores4 = mysql_query("SELECT * FROM `jogadores` WHERE avenda=1 ");

if($buscajogadores3 and $buscajogadores4) {
	echo "<xmlbacia>";
	
	while ($row = mysql_fetch_array($buscajogadores3)) {
		$num2++;
	}
	echo "<numeroJogadores>$num2</numeroJogadores>";
	
	while ($row = mysql_fetch_array($buscajogadores4)) {
		echo "<jogador$num>";
